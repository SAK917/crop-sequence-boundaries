...
def CSB_prep(file_path, shape_path, prep_path, CSByear, start_year, end_year):
    ...
    logging.basicConfig(
        filename=f"{prep_path}/log/{shapefile_name}.log",
    ...
    try:
        Merge_gdb = arcpy.management.CreateFileGDB(
            out_folder_path=merge_path, out_name=f"{shapefile_name}_CSB{CSByear}.gdb"
        )[0]
    ...
    while CF == False:
        try:
            arcpy.management.CalculateField(
                in_table=subregion, field="CSBYEARS", expression=CSByear, expression_type="PYTHON3"
            )
            CF = True
        except Exception as e:
            error_msg = e.args
    ...
    while spacialJoin_ads == False:
        try:
            arcpy.analysis.SpatialJoin(
                target_features=subregion,
                join_features=agdists,
                out_feature_class=subregion + "_ASD",
    ...
    while SpatialJoin == False:
        try:
            arcpy.analysis.SpatialJoin(
                target_features=subregion + "_ASD",
                join_features=CNTY,
                out_feature_class=subregion + "_CNTY1",
                join_operation="JOIN_ONE_TO_ONE",
                join_type="KEEP_ALL",
                field_mapping="",
                match_option="LARGEST_OVERLAP",
        ...
        while zonal == False:
            try:
                ZonalStatisticsAsTable(
                    in_zone_data=tif_raster_file,
                    zone_field="Value",
                    in_value_raster=rf"{national_cdl_folder}\\{y}\\{y}_30m_cdls.tif",
        ...
        while alterField == False:
            try:
                arcpy.management.AlterField(
                    ...
                    field="MAJORITY",
                    new_field_name="R" + str(f"{y}")[2:],
                    new_field_alias="R" + str(f"{y}")[2:],
                    field_type="LONG",
                    field_length=4,
                    field_is_nullable="NULLABLE",
...
if __name__ == "__main__":
    ...
    create_dir = utils.get_run_folder("prep", start_year, end_year)
    ...
    for shape_path in file_lst:
        ...
        p = multiprocessing.Process(
            target=CSB_prep, args=[file_path, shape_path, prep_dir, csb_year, start_year, end_year]
        )
        processes.append(p)
