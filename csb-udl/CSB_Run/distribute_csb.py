...
prep_dir = utils.get_run_folder("distribute", start_year, end_year)
...
logging.basicConfig(
    filename=distribute_dir + f"/log/CSB_Distribute.log",
...
national_gdb2 = arcpy.CreateFileGDB_management(distribute_dir + "//" + "National_Final_gdb", f"CSB{csb_year}" + ".gdb")[
    0
]
...
for gdb in file_lst:
    ...
    while process is False:
        try:
            final_fc = arcpy.management.AddGeometryAttributes(
                Input_Features=gdb + f"/{shapefile_name}_CNTY",
                Geometry_Properties=("AREA", "CENTROID_INSIDE"),
                Area_Unit="ACRES",
            )
            arcpy.management.AlterField(
                in_table=final_fc, field="POLY_AREA", new_field_name="CSBACRES", new_field_alias="CSBACRES"
            )
...
while calculate_field == False:
    try:
        arcpy.arcpy.management.CalculateField(
            in_table=national_gdb_lyr,
            field="CSBID",
...
for year in range(int(start_year), int(end_year) + 1, 1):
    crop_rotation_year_list.append(
        "" + "R" + (str(year)[2:]) + ' "' + "R" + (str(year)[2:]) + '" true true false 4 Long 0 0,First,#,'
        "" + fc + "," + "R" + (str(year)[2:]) + ",-1,-1;"
    )
...
while merge_process is False:
    try:
        arcpy.Merge_management(
            inputs=fc,
            output=national_gdb2 + "//" + "" + "national" + "GIS" + "",
            ...
            'CSBACRES "CSBACRES" true true false 8 Double 0 0,First,#,'
            + fc
            + ",CSBACRES,-1,-1;"
            + crop_rotation_year_str
            + 'STATEFIPS "STATEFIPS" true true false 2 Text 0 0,First,#,'
            + fc
            + ",STATEFIPS,0,2;"
...
# create a state gdb
state_gdb = arcpy.management.CreateFileGDB(
    out_folder_path=distribute_dir + r"\State_gdb", out_name=f"State_CSB{csb_year}.gdb"
)[0]
t7 = time.perf_counter()
# state distribution
for STATE in states:
    ...
    while state_distribute is False:
        try:
            ...
            select_state = arcpy.management.SelectLayerByAttribute(
                in_layer_or_view=national_gdb2 + "//" + "" + "national" + "GIS" + "",
                selection_type="NEW_SELECTION",
                where_clause=f"STATEFIPS = '{STFIPS}'",
                invert_where_clause="",
            )

            arcpy.FeatureClassToFeatureClass_conversion(select_state, state_gdb, f"CSB{STATE}{csb_year}")
...
for CSBpolyGIS in CSBpolyLISTGIS:
    ...
    arcpy.Select_analysis(in_features=CSBpolyGIS, out_feature_class=outShp)
